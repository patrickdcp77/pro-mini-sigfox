...
fonctionne en test POUR SIGFOX LORA
...
Serial.println("programme pour module sigfox LORA master sigfox code reference 20220211 precision balances optimisee");
...
Serial.println("AT+MODE=0"); // 0>sigfox   1>lora
Serial.flush() ;
Serial.println("AT$SH=8,12345678"); //8=nb d'octets envoyés
...
Serial.println("AT$SH=SETUP DONE");
...
//sprintf(Sigfox_message,"AT$SF=%02x%02x%02x%02x%04x%04x",header_byte,power_supply_voltage_Arduino,t_byte,h_byte,Weight_HX711_N1_Channel_A,Weight_HX711_N1_Channel_B);
sprintf(Sigfox_message,"AT$SH=12,%02x%02x%02x%02x%04x%04x",header_byte,power_supply_voltage_Arduino,t_byte,h_byte,Weight_HX711_N1_Channel_A,Weight_HX711_N1_Channel_B);
Serial.println(Sigfox_message);
Serial.flush() ;

//sleep mode non trouvé pour le module sigfox lora*****
//Serial.println("AT$P=2");   // SLeep mode
Serial.flush() ;
