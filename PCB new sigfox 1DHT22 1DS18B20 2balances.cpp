...
fonctionne correctement avec +++++++++PCB NEW (voir balances)++++++++++++
boitier avec module S15
+++++++++++++++++++++++++++++++++++++++++++++++
code rédigé pour 

2 balances
une sonde temp hum DHT22 (-35 à 50°)
une sonde tem DS18B20
...
alime+ de la DS18B20 sur D12  et DATA sur A3
sonde DHT22 alime en D10 et data en D11

panneau solaire et grosse batterie ION FER rechargeable
...
Operating voltage si pile plate ou rechargeable:
...
const unsigned int MAX_COUNTER_POWER_DOWN_WAKE_UP =2;// 116; //2 ; // MUST BE >=2 , 116 for 15mnsNumber of WATCH DOG before starting the main software  
...
if (counter_power_down_wake_up == 0) {
  ...
  if (isnan(h) || isnan(t)) {
    ...
    if (DEBUG_MODE) { 
                        ...
                        Serial.print(td_byte,HEX);
                        Serial.println(" Byte   ");
                        
                           
                        Serial.print("Hum DHT : "); 
                        Serial.print(h);
                        Serial.print(" %\t SIGFOX #05: ");
                        Serial.print(hd_byte,HEX);
                        Serial.println(" Byte   ");
...
sprintf(Sigfox_message,"AT$SF=%02x%02x%02x%02x%02x%04x%04x",header_byte,power_supply_voltage_Arduino,t_byte ,td_byte,hd_byte,Weight_HX711_N1_Channel_A,Weight_HX711_N1_Channel_B);
